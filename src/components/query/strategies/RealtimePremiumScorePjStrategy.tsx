  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 border-l-4 border-l-purple-500 bg-white shadow-lg">
           <div className="flex flex-col md:flex-row gap-6">
             <div className="flex-1">
               <div className="flex items-center gap-2 mb-3">
                 <Badge variant={data.company.status === 'ATIVA' ? 'success' : 'warning'} size="md">
                   {data.company.status}
                 </Badge>
               ...
               <p className="text-sm text-gray-500 mb-4">{data.company.socialReason}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.partners.map((partner, idx) => (
                <div key={idx} className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-100">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 text-sm line-clamp-1" title={partner.name}>{partner.name}</p>
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                       <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-medium text-[10px]">{partner.role}</span>
